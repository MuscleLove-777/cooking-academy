/* ============================================
   Cooking Academy - Main Application
   ============================================ */
...
const App = {
    ...
    init() {
        this.levels = [LEVEL1_DATA, LEVEL2_DATA, LEVEL3_DATA, LEVEL4_DATA, LEVEL5_DATA, LEVEL6_DATA, LEVEL7_DATA, LEVEL8_DATA, LEVEL9_DATA, LEVEL10_DATA, LEVEL11_DATA, LEVEL12_DATA];
        ...
        if (localStorage.getItem('cooking-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('cooking-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('cooking-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('cooking-progress', JSON.stringify(this.progress));
        localStorage.setItem('cooking-quiz-results', JSON.stringify(this.quizResults));
    },
    ...
    buildSidebar() {
        ...
        this.levels.forEach(level => {
            ...
            levelModules.forEach(mod => {
                ...
                html += '<div class="sidebar-item ' + (isCompleted ? 'completed' : '') + ' ' + (isActive ? 'active' : '') + '"' +
                    ' onclick="App.showModule(' + mod.id + ')">' +
                    '<span class="status-dot"></span>' +
                    '<span>' + mod.title + '</span></div>';
            });
    ...
    showDashboard() {
        ...
        var html = '<div class="fade-in">' +
            ...
            '<h2>料理 Academy へようこそ</h2>' +
            '<p>包丁の持ち方から献立設計まで、料理スキルを体系的に学べるプラットフォームです。</p>' +
            '</div>' +
        ...
        this.levels.forEach(level => {
            ...
            html += '<div class="level-card level-' + level.id + '" onclick="App.showModule(' + levelMods[0].id + ')">' +
                ...
                '<div><h3>Level ' + level.id + ': ' + level.title + '</h3>' +
                '<div class="level-desc">' + level.description + ' (' + levelMods.length + 'モジュール)</div></div></div>' +
    ...
    showModule(moduleId) {
        ...
        if (prevMod) {
            html += '<button class="btn btn-outline" onclick="App.showModule(' + prevMod.id + ')">&larr; ' + prevMod.title + '</button>';
        }

        html += '</div><div style="display:flex;gap:12px;">';

        if (mod.quiz && mod.quiz.length > 0) {
            html += '<button class="btn btn-primary btn-lg" onclick="App.startQuiz(' + moduleId + ')">理解度チェック (' + mod.quiz.length + '問)</button>';
        } else {
            html += '<button class="btn btn-success btn-lg" onclick="App.completeModule(' + moduleId + '); App.goToNextModule(' + moduleId + ');">完了して次へ</button>';
        }

        html += '</div><div>';

        if (nextMod) {
            html += '<button class="btn btn-outline" onclick="App.showModule(' + nextMod.id + ')">' + nextMod.title + ' &rarr;</button>';
        }
    ...
    showCompletionMessage() {
        ...
        modal.innerHTML = '<h2>&#127891; おめでとうございます！</h2>' +
            ...
            '<button class="btn btn-primary" onclick="App.closeModal()">ダッシュボードへ</button></div>';
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('cooking-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('cooking-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('cooking-progress');
            localStorage.removeItem('cooking-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
